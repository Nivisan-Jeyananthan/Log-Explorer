...
int db_add_tag(DB *d, int log_id, const char *tag) {
    ...
    if (sqlite3_prepare_v2(d->db, ins_tag, -1, &stmt, NULL) != SQLITE_OK) return -1;
    ...
    if (sqlite3_prepare_v2(d->db, get_tagid, -1, &stmt, NULL) != SQLITE_OK) return -1;
    sqlite3_bind_text(stmt, 1, tag, -1, SQLITE_STATIC);
    int tag_id = -1;
    ...
    if (sqlite3_prepare_v2(d->db, link, -1, &stmt, NULL) != SQLITE_OK) return -1;
...
int db_remove_tag(DB *d, int log_id, const char *tag) {
    ...
    if (sqlite3_prepare_v2(d->db, del, -1, &stmt, NULL) != SQLITE_OK) return -1;
    sqlite3_bind_int(stmt, 1, log_id);
    sqlite3_bind_int(stmt, 2, tag_id);
    if (sqlite3_step(stmt) != SQLITE_DONE) { sqlite3_finalize(stmt); return -1; }
    sqlite3_finalize(stmt);
...
char **db_list_tags(DB *d, int log_id) {
    ...
    if (sqlite3_prepare_v2(d->db, sql, -1, &stmt, NULL) != SQLITE_OK) return NULL;
    sqlite3_bind_int(stmt, 1, log_id);
    char **arr = NULL;
    size_t n = 0;
...
int db_insert_log(DB *d, const char *source, const char *unit, const char *message, const char *ts) {
    ...
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(d->db, sql, -1, &stmt, NULL) != SQLITE_OK) return -1;
...
int db_search(DB *d, const char *query, int limit, int offset, sqlite3_stmt **out_stmt) {
    if (!query || query[0] == '\0') {
        ...
        if (sqlite3_prepare_v2(d->db, sql_recent, -1, out_stmt, NULL) != SQLITE_OK) return -1;
    ...
    if (sqlite3_prepare_v2(d->db, sql, -1, out_stmt, NULL) != SQLITE_OK) return -1;
